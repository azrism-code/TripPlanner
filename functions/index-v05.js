...
async function buildTripContext(tripId, trip) {
  const [takeItems, hotels, cars, flights, expenses, itineraryDays, documents] = await Promise.all([
...
async function executeTool(tripId, call) {
  ...
  if (call.name === 'add_hotel') {
    await tripRef.collection('hotels').add({
      ...
      bookingRef: clean(args.bookingRef), notes: clean(args.notes), createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()
    })
    return { ok: true }
  }
  ...
  if (call.name === 'add_car') {
    await tripRef.collection('cars').add({
      company: clean(args.company), pickup: clean(args.pickup), dropoff: clean(args.dropoff), pickupDate: clean(args.pickupDate),
      dropoffDate: clean(args.dropoffDate), bookingRef: clean(args.bookingRef), notes: clean(args.notes), createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()
    })
    return { ok: true }
  }
  ...
  if (call.name === 'add_flight') {
    await tripRef.collection('flights').add({
      airline: clean(args.airline), flightNumber: clean(args.flightNumber), from: clean(args.from), to: clean(args.to),
      departureDate: clean(args.departureDate), departureTime: clean(args.departureTime), arrivalDate: clean(args.arrivalDate), arrivalTime: clean(args.arrivalTime),
      bookingRef: clean(args.bookingRef), notes: clean(args.notes), createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp()
    })
    return { ok: true }
  }
...
export const tripChat = onRequest({
  ...
  try {
    ...
    const instructions = `You are Trip AI inside a travel-planning app. The authenticated user is the OWNER of this trip.\n\nUse trip data and uploaded documents as the source of truth. Extract booking details from uploaded flight tickets, hotel confirmations and car-rental confirmations when the owner asks you to import or use them. Never invent confirmation numbers, flight details, hotel bookings or dates.\n\nYou may use write tools only when the owner explicitly asks to add, change, import, build or correct something. When building itinerary days, stay inside the trip start/end dates and respect known flight, hotel and car timings. Keep replies concise and practical. Reply in Hebrew unless the user writes in another language.`
