...
function removeOldFlightItems(items, flight, groupIds, sourceIds, key) {
  ...
  return items.filter((item) => {
    ...
    if (type === 'flight' && flightNo && numberKey(item.title).includes(flightNo)) return false
    return true
  })
}
...
async function rewriteItinerary(tripRef, trip, flight, allFlights, group, key) {
  ...
  for (const [date, day] of days) day.items = removeOldFlightItems(day.items, flight, groupIds, sourceIds, key)
...
export const normalizeImportedFlight = onDocumentWritten({
...
export const normalizeFlightData = onCall({ region: 'europe-west1', timeoutSeconds: 180, memory: '512MiB' }, async (request) => {
